...
import {
  ...
  removeOrderItemQtyAction,
} from "../../../redux/slice/cart/cartSlice";
import { current } from "@reduxjs/toolkit";
import { fetchSingleCouponAction } from "../../../redux/slice/coupons/couponsSlice";
...
export default function ShoppingCart() {
  ...
  //calculate total price (calculate the total price and display it in order summary)
  let sumTotalPrice = 0;
  ...
  //price of the product - {price of product * discount/100}

  //! remove order item  handler
  const removeOrderItemFromLocalStorageHandler = (productId) => {
    dispatch(removeOrderItemQtyAction(productId));
    dispatch(getCartItemsFromLocalStorageAction());
  };
                    ...
                    <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-900">
                          RS {product?.price} * {product?.qty} = Rs{" "}
                          {product?.totalPrice}
                        </p>
                      </div>
                      ...
                      <div className="mt-4 sm:mt-0 sm:pr-9">
                        ...
                        >
                          {/* use the qty  to display the qty left dynamically we are using JS Array method */}
